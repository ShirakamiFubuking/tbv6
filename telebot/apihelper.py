...
def _make_request(token, method_name, method='get', params=None, files=None, base_url=API_URL):
    while True:
        try:
            return __make_request(token, method_name, method=method, params=params, files=files, base_url=base_url)
        except ApiException as e:
            logger.warning(e)
            if e.result.status_code == 429:
                body = e.result.json()
                logger.error(f"429 error,retry after {body['retry_after']} seconds.")
                time.sleep(body['retry_after'])
            else:
                raise


def __make_request(token, method_name, method='get', params=None, files=None, base_url=API_URL):
    """
