...
msg_handler = namedtuple('msg_handler', 'func filter')
...
class TeleBot:
    ...
    def __init__(self, token, threaded=True, skip_pending=False, num_threads=2):
        ...
        # key: (chat id,user id) value:UserSpaceHandler
        self.user_space = {}
    ...
    def promote_chat_member(self, chat_id, user_id, can_change_info=None, can_post_messages=None,
        ...
        :param can_edit_messages: Bool : Pass True, if the administrator can edit messages of other admin, channels only
        :param can_delete_messages: Bool : Pass True, if the administrator can delete messages of other admin
        :param can_invite_users: Bool : Pass True, if the administrator can invite new admin to the chat
    ...
    def send_poll(self, chat_id, poll, disable_notifications=False, reply_to_message=None, reply_markup=None):
        return types.Message.de_json(
            apihelper.send_poll(self.token, chat_id, poll.question, poll.options, disable_notifications,
    ...
    @staticmethod
    def _build_handler_dict(handler, func):
        return msg_handler(handler, func)
    ...
    def message_handler(self, commands=None, regexp=None, func=None, content_types=('text',)):
        ...
        content_type can be one of the following strings: text, audio, document, photo, sticker, video,
        video_note, voice, location, contact, new_chat_members, left_chat_member, new_chat_title,
        ...
        def decorator(handler):
            filters = util.exec_filters(util.bot_commands(commands),
                                        util.match_reg(regexp),
                                        util.check_msg_type(content_types),
                                        func)
    ...
    def channel_post_handler(self, commands=None, regexp=None, func=None, content_types=('text',)):
        def decorator(handler):
            filters = util.exec_filters(util.bot_commands(commands),
                                        util.match_reg(regexp),
                                        util.check_msg_type(content_types),
    ...
    def edited_channel_post_handler(self, commands=None, regexp=None, func=None, content_types=('text',)):
        def decorator(handler):
            filters = util.exec_filters(util.bot_commands(commands),
                                        util.match_reg(regexp),
                                        util.check_msg_type(content_types),
    ...
    def inline_handler(self, func):
        def decorator(handler):
            handler_dict = self._build_handler_dict(handler, func)
    ...
    def shipping_query_handler(self, func):
        def decorator(handler):
            handler_dict = self._build_handler_dict(handler, func)
    ...
    @staticmethod
    def exec_task(func, message):
        func(message)

    def _notify_command_handlers(self, handlers, new_messages):
        for message in new_messages:
            for message_handler in handlers:
                if message_handler.filter(message):
                    self._exec_task(self.exec_task, message_handler.func, message)
                    break
